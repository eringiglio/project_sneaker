...
for (j in 1:ITERATIONS){
  ...
  for (i in 1:GENERATIONS){
    ...
    #LINES 75-94 COMMENTED OUT TO PROVIDE DIFFERENTIAL MATING SUCCESS
    ...
    #Number of matings for each male genotype
    #0.9 is a constant - change eventually to some parameter/function
    NUMBER_MATINGS_TERR<-rmultinom(1,ceiling(0.9*TOTAL_FEMALE),c(NUMBERS_MALE[1]/TOTAL_TERR,NUMBERS_MALE[2]/TOTAL_TERR))
    NUMBER_MATINGS_TT<-NUMBER_MATINGS_TERR[1]
    ...
    TT_TO_ST<-(NUMBER_MATINGS_SS/TOTAL_FEMALE)+(0.5*NUMBER_MATINGS_ST/TOTAL_FEMALE)
    ST_TO_TT<-(0.5*NUMBER_MATINGS_TT/TOTAL_FEMALE)+(0.25*NUMBER_MATINGS_ST/TOTAL_FEMALE)
    ST_TO_SS<-(0.5*NUMBER_MATINGS_SS/TOTAL_FEMALE)+(0.25*NUMBER_MATINGS_ST/TOTAL_FEMALE)
    ST_TO_ST<-(1-ST_TO_TT-ST_TO_SS)
